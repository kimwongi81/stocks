...
<%
	...
	//종목 링크 구하기.
	String[] itmLinkList = new String[150];
	int idx = 0;
	
	url = "https://finance.naver.com/sise/sise_market_sum.nhn?&page=1";
	Document doc = Jsoup.connect(url).get();
	Elements elm = doc.select("div[class=box_type_l] table td a[href]");
	
	for(int i=0; elm.size() > i; i++ ){
		String href = elm.get(i).attr("href");
		if(href.indexOf("/item/main.naver") > -1){
			itmLinkList[idx] = elm.get(i).attr("href");
			idx++;	
		}
	}
	...
	elm = doc.select("div[class=box_type_l] a[href]");
...
<%	
	for(int i=0; itmLinkList.length > i; i++ ){
		...
		if(StringUtils.isEmpty(stcQt)){
			stcQt = "1";
		}
		System.out.println("주식수 : " + stcQt);
		...
		//ROE 분기
		Double roeQuarter1 = Double.parseDouble(StringUtils.defaultIfEmpty(doc2.select("div[class=sub_section] table.tb_type1_ifrs tbody tr td").eq(57).text().replaceAll(",", "").replaceAll(" ", ""),"0"));	//영업이익 작년
		if("-".equals(roeQuarter1)){
			roeQuarter1 = 0.0;
		}
		Double roeQuarter2 = Double.parseDouble(StringUtils.defaultIfEmpty(doc2.select("div[class=sub_section] table.tb_type1_ifrs tbody tr td").eq(58).text().replaceAll(",", "").replaceAll(" ", ""),"0"));	//영업이익 작년
		if("-".equals(roeQuarter2)){
			roeQuarter2 = 0.0;
		}
...
%>
			<tr>
				<td><%=i+1%></td>
				<td><a href="<%=url2%>" target="_blank"><%=companyName %></a></td>
				<td>
<%
					if(py > marketSum){
%>
