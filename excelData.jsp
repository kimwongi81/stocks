...
<%
	String excelPath = request.getParameter("excel");
	String fullPath = "C:/10/"+excelPath;
    ...
    String[] itmLinkList = new String[rows-1];
    int idx = 0;
        ...
        if(row !=null){
            ...
            for(columnindex=0; columnindex<=cells; columnindex++){
                ...
                if(columnindex == 0){
                	...
                	itmLinkList[idx] = "item/main.nhn?code="+value;
                	System.out.println("itmLinkList[idx] : "+ idx + " // : " + itmLinkList[idx]);
                	idx++;
                }
            }
...
<table id="table" class="tablesorter">
	...
	<tbody>
<%	
	System.out.println("itmLinkList.length : " + itmLinkList.length);
	for(int i=0; itmLinkList.length > i; i++ ){
		
		//종목별 데이터 구하기.
		String url2 = "https://finance.naver.com/"+itmLinkList[i];
		
		System.out.println("itmLinkList[i] : "+ i + " / " + itmLinkList[i]);
		...
		//ROE 분기
		Double roeQuarter1 = 0.0;	
		String roeQuarter1Str = StringUtils.defaultIfEmpty(doc2.select("div[class=sub_section] table.tb_type1_ifrs tbody tr td").eq(57).text().replaceAll(",", "").replaceAll(" ", ""),"0");	//영업이익 작년
		if(!"-".equals(roeQuarter1Str)){
			roeQuarter1 = Double.parseDouble(roeQuarter1Str);
		}
		
		Double roeQuarter2 = 0.0;
		String roeQuarter2Str = StringUtils.defaultIfEmpty(doc2.select("div[class=sub_section] table.tb_type1_ifrs tbody tr td").eq(58).text().replaceAll(",", "").replaceAll(" ", ""),"0");	//영업이익 작년
		if(!"-".equals(roeQuarter2Str)){
			roeQuarter2 = Double.parseDouble(roeQuarter2Str);
		}
		
		Double roeQuarter3 = 0.0;
		String roeQuarter3Str = StringUtils.defaultIfEmpty(doc2.select("div[class=sub_section] table.tb_type1_ifrs tbody tr td").eq(59).text().replaceAll(",", "").replaceAll(" ", ""),"0");	//영업이익 작년
		if("-".equals(roeQuarter3Str)){
			roeQuarter3 = Double.parseDouble(roeQuarter3Str);
		}
		if(roeYear3 == 0){
		...
		System.out.println("profitYear2 : " + profitYear2);
		System.out.println("stcQt : " + stcQt);
...
<%
					if(py > marketSum){
%>
					<font color='red'><%=String.format("%,.0f", marketSum) %></font>
<%					
					}else{
%>
...
<%
	...
	//분기 영업이익
	Long quarter1 = Double.parseDouble(StringUtils.defaultIfEmpty(doc2.select("div[class=sub_section] table.tb_type1_ifrs tbody tr td").eq(17).text().replaceAll(",", "").replaceAll(" ", ""),"0"));	//영업이익 분기
	Long quarter2 = Double.parseDouble(StringUtils.defaultIfEmpty(doc2.select("div[class=sub_section] table.tb_type1_ifrs tbody tr td").eq(18).text().replaceAll(",", "").replaceAll(" ", ""),"0"));	//영업이익 분기
	Long quarter3 = Double.parseDouble(StringUtils.defaultIfEmpty(doc2.select("div[class=sub_section] table.tb_type1_ifrs tbody tr td").eq(19).text().replaceAll(",", "").replaceAll(" ", ""),"0"));	//영업이익 분기
	if(quarter3 == 0){
		quarter3 = (quarter1+quarter2)/2;
	}
	...
	//연도별 영업이익
	//Long profitLastYear = Double.parseDouble(StringUtils.defaultIfEmpty(doc2.select("div[class=sub_section] table.tb_type1_ifrs tbody tr td").eq(12).text().replaceAll(",", "").replaceAll(" ", ""),"0"));	//영업이익 작년
	Long profitYear2 = Double.parseDouble(StringUtils.defaultIfEmpty(doc2.select("div[class=sub_section] table.tb_type1_ifrs tbody tr td").eq(13).text().replaceAll(",", "").replaceAll(" ", ""),"0"));	//영업이익 당해
	if(profitYear2 == 0){
		profitYear2 = (quarter1 + quarter2 + quarter3)/3;
	}
	
	//ROE 연간
	Float roeYear1 = Double.parseDouble(StringUtils.defaultIfEmpty(doc2.select("div[class=sub_section] table.tb_type1_ifrs tbody tr td").eq(51).text().replaceAll(",", "").replaceAll(" ", ""),"0"));	//영업이익 작년
	Float roeYear2 = Double.parseDouble(StringUtils.defaultIfEmpty(doc2.select("div[class=sub_section] table.tb_type1_ifrs tbody tr td").eq(52).text().replaceAll(",", "").replaceAll(" ", ""),"0"));	//영업이익 작년
	Float roeYear3 = Double.parseDouble(StringUtils.defaultIfEmpty(doc2.select("div[class=sub_section] table.tb_type1_ifrs tbody tr td").eq(53).text().replaceAll(",", "").replaceAll(" ", ""),"0"));	//영업이익 작년
	
	//ROE 분기
	Float roeQuarter1 = Double.parseDouble(StringUtils.defaultIfEmpty(doc2.select("div[class=sub_section] table.tb_type1_ifrs tbody tr td").eq(57).text().replaceAll(",", "").replaceAll(" ", ""),"0"));	//영업이익 작년
	Float roeQuarter2 = Double.parseDouble(StringUtils.defaultIfEmpty(doc2.select("div[class=sub_section] table.tb_type1_ifrs tbody tr td").eq(58).text().replaceAll(",", "").replaceAll(" ", ""),"0"));	//영업이익 작년
	Float roeQuarter3 = Double.parseDouble(StringUtils.defaultIfEmpty(doc2.select("div[class=sub_section] table.tb_type1_ifrs tbody tr td").eq(59).text().replaceAll(",", "").replaceAll(" ", ""),"0"));	//영업이익 작년
	if(roeYear3 == 0){
		roeYear3 = (roeQuarter1 + roeQuarter2 + ((roeQuarter1 + roeQuarter2)/2))/3;
	}
	...
	function setMt(){
		$(".py").each(function(){
			...
			if(mtVal > price){
				...
				$("#mt"+idNum).html("<font color='red'>"+numberWithCommas(mtVal)+ " (" + parseInt(per)  + "%)</font>");
			}else{
